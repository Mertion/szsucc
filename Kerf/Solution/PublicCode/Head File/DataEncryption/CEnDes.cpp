...
void CEnDes::pack8(BYTE *packed, BYTE *binary)
{
   register WORD i, j, k;
...
void CEnDes::unpack8(BYTE *packed, BYTE *binary)
{
   register WORD i, j, k;
...
//异或
void CEnDes::Do_XOR(unsigned char *dest, unsigned char *source, int size)
{
    int i;
    for (i=0;i<size;i++)
    dest[i]^=source[i];
...
//异或
void CEnDes::BitXor(BYTE *Data1, BYTE *Data2, int Len, BYTE *Dest)
{
	int i;
	for (i = 0; i < Len; i++)	
...
void CEnDes::DES_setkey(BYTE sw, BYTE *pkey)
{
...
//DES计算
void CEnDes::DES_calcul(BYTE *in, BYTE *out)
{
...
void CEnDes::DES(BYTE Decrypt, BYTE *Key, BYTE *Data, BYTE *Result)
{
...
void CEnDes::Triple_DES(BYTE Decrypt, BYTE* Key, BYTE* Data, BYTE* Result)
{
   BYTE  Tmp1[16],
         Tmp2[16];
