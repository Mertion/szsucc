...
{
	memset(pHandle, 0, sizeof(int) * 16);
	synchronizeflag = 0;
	hHandle = nullptr;
}
...
bool CSerialPort::open(const char* portname, int baudrate, char parity, char databit, char stopbit, char synchronizeflag)
{
	...
	TimeOuts.ReadTotalTimeoutMultiplier = 2; //读时间系数
	TimeOuts.ReadTotalTimeoutConstant = 1; //读时间常量
	TimeOuts.WriteTotalTimeoutMultiplier = 1; // 写时间系数
	TimeOuts.WriteTotalTimeoutConstant = 2; //写时间常量
...
int CSerialPort::send(string dat)
{
	...
	{
		// 同步方式
		DWORD dwBytesWrite = (int)dat.length(); //成功写入的数据字节数
	...
	else
	{
		...
		if (!bWriteStat)
		{
			...
			{
				if (m_osWrite.hEvent!=nullptr)
				{
					WaitForSingleObject((HANDLE)m_osWrite.hEvent, 1000); //等待写入事件1秒钟
				}
			}
			else
			{
				...
				if (m_osWrite.hEvent != nullptr)
				{
					CloseHandle((HANDLE)m_osWrite.hEvent); //关闭并释放hEvent内存
				}
				return 0;
			}
		}
...
string CSerialPort::receive()
{
	...
	else
	{
		...
		if (!bReadStat)
		{
			...
			{
				ClearCommError(hCom, &dwErrorFlags, &comStat); //清除通讯错误
				if (m_osRead.hEvent!=nullptr)
				{
					CloseHandle((HANDLE)m_osRead.hEvent); //关闭并释放hEvent的内存
				}
...
int CSerialPort::GetConnectionStatus()
{
	//获得串口连接状态
	if (hHandle != nullptr)
	{
